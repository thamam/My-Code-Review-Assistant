...
export const CodeViewer: React.FC = () => {
  ...
  useEffect(() => {
    if (selectedFile) {
      const timer = setTimeout(() => setIsCodeViewerReady(true), 150);
  ...
  useEffect(() => {
      ...
      if (targetLine !== null) {
          ...
          setTimeout(() => {
              let el = document.querySelector(`[data-line-id="${selectedFile.path}:${lineNum}"]`) || document.querySelector(`[data-line-number="${lineNum}"]`);
              if (el) {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-950 overflow-hidden relative">
      <style>{`.navigation-highlight { background-color: rgba(59, 130, 246, 0.4) !important; ring: 2px solid #60a5fa; transition: background-color 2s ease; }`}</style>
      <div className="flex items-center justify-between p-3 border-b border-gray-800 bg-gray-900 shrink-0">
        ...
        <div className="flex gap-2">
            {selectedFile.path.endsWith('.md') && <button onClick={() => setIsPreviewMode(!isPreviewMode)} className="p-1 hover:text-white"><Eye size={16} /></button>}
        </div>
      </div>
      <div ref={containerRef} className="flex-1 overflow-auto custom-scrollbar relative">
        {isPreviewMode && selectedFile.path.endsWith('.md') ? <div className="p-8"><MarkdownRenderer content={selectedFile.newContent} /></div> :
         isSource ? <SourceView key={selectedFile.path} content={selectedFile.newContent || selectedFile.oldContent || ""} filePath={selectedFile.path} /> :
         <DiffView key={selectedFile.path} oldContent={selectedFile.oldContent} newContent={selectedFile.newContent} filePath={selectedFile.path} onViewportChange={(f, s, e) => updateViewport({file: f, startLine: s, endLine: e})} />}
      </div>
    </div>
  );
};
